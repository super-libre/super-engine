#!/bin/bash
# End-to-end install verification: performs a REAL install of a product's
# REAL published release onto THIS machine, asserts the complete installed
# tree, then runs the product's uninstall.sh and asserts nothing is left
# behind.
#
# This is the counterpart to a product's scripts/test-install.sh, which only
# covers install.sh's pure, network-free logic (arch detection, channel
# validation, tag resolution from fixture JSON). Nothing there — and nothing
# in the installer's own `--dry-run` e2e test — ever writes a file, escalates,
...
#      release.
#   2. Local pass. The same install driven by a locally built installer
#      ($<PREFIX>_INSTALLER_BIN), so changes to the installer in a PR are what
#      is under test rather than whatever the last release shipped.
#
...
# The trade-off is that a change to the dist layout (a new resource in the
...
#       bash test-install-e2e.sh <slug> <short name> [stable|beta]
#
# <slug> and <short name> are the product's, as in super-engine-protocol's
# Product: `super-stt stt` for Super STT. <PREFIX> is the slug in upper case,
# with `_` for `-` (SUPER_STT). GITHUB_REPOSITORY names the repo whose releases
# are installed, and Actions sets it. CI runs this through action.yml next to
# it; a product runs it locally as `just test-install-e2e [stable|beta]`.
...
if ! [[ "$SLUG" =~ ^[a-z][a-z0-9-]*$ && "$SHORT_NAME" =~ ^[a-z][a-z0-9-]*$ ]]; then
    err "usage: test-install-e2e.sh <slug> <short name> [stable|beta]"
    err "The slug and short name are lower case, as in super-stt and stt."
    exit 2
fi
PREFIX="${SLUG^^}"
PREFIX="${PREFIX//-/_}"
YES_VAR="${PREFIX}_INSTALL_E2E_YES"
INSTALLER_VAR="${PREFIX}_INSTALLER_BIN"
...
# that has the product installed. Opting in is explicit and never defaulted.
if [ "${!YES_VAR:-}" != "1" ]; then
    err "This test performs a REAL install into /usr/local and /usr/lib/systemd/user,"
    err "then uninstalls it. It will remove an existing $SLUG install on this host."
    err ""
    err "Run it on a disposable machine (CI) or in a container, and opt in explicitly:"
    err "  $YES_VAR=1 just test-install-e2e [stable|beta]"
    exit 2
fi
...
if [ -z "$GITHUB_REPO" ]; then
    err "Set GITHUB_REPOSITORY to the <owner>/<repo> whose releases to install."
    exit 2
fi
if [ ! -f "$REPO_ROOT/install.sh" ] || [ ! -f "$REPO_ROOT/uninstall.sh" ]; then
    err "Run this from the root of the product's checkout."
    err "There is no install.sh and uninstall.sh in $REPO_ROOT."
    exit 2
fi
...
# super-engine-installer's stage::build_manifest. The short-name wrapper has
# no source in the tarball — build_manifest generates it — and
# `<slug>-install` is the installer copying its own binary into place; both
# are as much a part of a complete install as anything unpacked from the
# release.
...
assert_installed_tree() {
    ...
    # The unit is what `systemctl --user enable <slug>` will read; a
    # truncated or mangled copy would still satisfy the existence check above.
    if grep -q '^\[Service\]' "$UNIT" \
        && grep -q '^ExecStart=' "$UNIT"; then
        ...
        fail "$label: the systemd unit has a [Service] section with ExecStart" \
            "a parseable unit" "$(head -5 "$UNIT" 2>&1)"
    fi
    ...
    cli_out=$("/usr/local/bin/$SLUG-cli" --version 2>&1)
    version="${cli_out#"$SLUG-cli "}"
    if [ "$cli_out" != "$version" ] && is_accepted "v$version" "$accepted"; then
        ...
        fail "$label: the installed CLI runs and reports its version" \
            "$SLUG-cli <$(accepted_desc "$accepted")>" "$cli_out"
    fi

    # The generated short-name wrapper is what keyboard shortcuts invoke; a
    # wrapper pointing at the wrong prefix would be invisible to every check
    # above.
    local wrapper_out
    wrapper_out=$("/usr/local/bin/$SHORT_NAME" --version 2>&1)
    if [ "$wrapper_out" = "$cli_out" ]; then
        pass "$label: the $SHORT_NAME wrapper execs the installed CLI"
    else
        fail "$label: the $SHORT_NAME wrapper execs the installed CLI" "$cli_out" "$wrapper_out"
    fi
}
...
local_pass() {
    ...
    if [ -z "$INSTALLER_BIN" ]; then
        skip "local pass: set $INSTALLER_VAR to a built $SLUG-install to run it"
        return
    fi
    if [ ! -x "$INSTALLER_BIN" ]; then
        fail "local pass: $INSTALLER_VAR is executable" \
...
if [ "${#PREEXISTING[@]}" -ne 0 ]; then
    err "$SLUG is already installed on this host — refusing to run:"
    printf '        %s\n' "${PREEXISTING[@]}"
...
# the product's scripts/test-install.sh.
