...
//! manifest asset + the binary asset download; run the indexer's `build`
//! against it via the `GITHUB_API_BASE` env override.
//!
//! This is one happy-path test, and the only test in this binary: it sets a
//! process-wide environment variable, which is only sound while nothing else
//! runs beside it. Pure-unit tests cover failure cases in each module's
//! `#[cfg(test)]` block.
