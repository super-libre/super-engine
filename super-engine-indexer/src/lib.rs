...
//! A product's backend registry indexer.
//!
//! It reads `registry.toml`, resolves each entry's latest release on its
//! forge, validates the release's `backend.toml`, hashes its artifacts, and
//! writes the `index.json` a daemon lists and installs backends from.
//!
//! A product supplies its [`Product`] type, which decides the manifests it
//! parses and the index fields it publishes, and an [`Indexer`]. Its indexer
//! binary is then one call to [`main`].
//!
//! Two subcommands:
//!
//! - `build`: the published index, from `registry.toml` and forge releases.
//! - `local`: an index over backends staged on disk, offline, for testing a
//!   daemon's install pipeline against a static server.
...
use super_engine_spec::index::{
    Index, IndexAsset, IndexAssets, IndexBackend, IndexSubprocessAsset, MIN_CLIENT, SCHEMA_VERSION,
};
use super_engine_spec::manifest::{Manifest, SubprocessAsset};
...
/// What a product tells the indexer about itself.
pub struct Indexer {
    /// The binary's name, as its help and errors print it.
    pub name: &'static str,
    /// What the indexer calls itself to forges and download hosts.
    pub user_agent: &'static str,
    /// Registry keys that may still omit `id`. See
    /// [`registry_toml::Registry::parse`].
    pub grandfathered: &'static [&'static str],
}
...
struct BuildFailure {
    ...
    attempted_tag: Option<String>,
}

/// Run as a product's indexer binary: set up TLS and logging, read the
/// command line, and do what it says.
///
/// # Errors
/// Whatever the command fails with.
pub async fn main<P: Product>(indexer: &Indexer) -> anyhow::Result<()> {
    ...
    let matches = Args::command()
        .name(indexer.name)
        .bin_name(indexer.name)
        .get_matches();
    run::<P>(indexer, Args::from_arg_matches(&matches)?).await
}
...
/// # Errors
/// Whatever the command fails with.
pub async fn run<P: Product>(indexer: &Indexer, args: Args) -> anyhow::Result<()> {
    match args.command {
        Command::Build(args) => run_build::<P>(indexer, args).await,
        Command::Local(args) => local::run::<P>(&args),
    }
}

/// Build the published index from `registry.toml` + GitHub releases.
async fn run_build<P: Product>(indexer: &Indexer, args: BuildArgs) -> anyhow::Result<()> {
    let registry_text = std::fs::read_to_string(&args.registry)
        .with_context(|| format!("reading {}", args.registry.display()))?;
    let registry = registry_toml::Registry::parse(&registry_text, indexer.grandfathered)?;

    let prior = match args.prior_index.as_ref() {
        Some(p) if p.exists() => {
            let text = std::fs::read_to_string(p)?;
            Some(serde_json::from_str::<Index<P::IndexModel>>(&text)?)
        }
        _ => None,
    };
    ...
    let http = super_engine_forge::http::download_client(indexer.user_agent);
    let now_iso = chrono_now_iso();

    let mut out_backends: Vec<IndexBackend<P::IndexModel>> = Vec::new();
    ...
    for (id, entry) in &registry.0 {
        ...
        let client = super_engine_forge::client(entry.forge, indexer.user_agent);
        ...
        let built = match RepoRef::parse(&entry.repo) {
            Ok(repo) => build_entry::<P>(client.as_ref(), &http, id, entry, &repo).await,
    ...
    super_engine_spec::fs::write_atomic(&args.out, text.as_bytes())
...
async fn build_entry<P: Product>(
    client: &dyn ForgeClient,
    http: &reqwest::Client,
    id: &str,
    entry: &registry_toml::Entry,
    repo: &RepoRef,
) -> Result<IndexBackend<P::IndexModel>, BuildFailure> {
    ...
    let m = Manifest::<P>::parse(&text).map_err(|e| fail(&e))?;
    let manifest_pin = Some(IndexAsset { url, size, sha256 });
...
fn temp_part_path() -> std::path::PathBuf {
    ...
    std::env::temp_dir().join(format!("backend-index-{}-{n}.part", std::process::id()))
}
...
fn subprocess_index_entry(sa: &SubprocessAsset, mut pins: Vec<IndexAsset>) -> IndexSubprocessAsset {
...
async fn resolve_index_assets<P: Product>(
    http: &reqwest::Client,
    m: &Manifest<P>,
    release_assets: &[super_engine_forge::ReleaseAsset],
) -> anyhow::Result<IndexAssets> {
    let mut idx_assets = IndexAssets::default();
    if let Some(wasm) = &m.assets.wasm {
        ...
        idx_assets.wasm = Some(IndexAsset {
            url,
            size,
            sha256: sha,
        });
    ...
    for sa in &m.assets.subprocess {
        ...
        for f in &files {
            ...
            pins.push(IndexAsset { url, size, sha256 });
        }
...
/// canonical [`IndexBackend::from_manifest`] synthesis (shared with the
/// daemon's Custom-repo and local-dir install paths) — the indexer supplies
/// the maintainer-declared `id` rather than deriving it from `source`.
pub(crate) fn into_index_backend<P: Product>(
    id: &str,
    m: Manifest<P>,
    version: String,
    tag: String,
...
) -> IndexBackend<P::IndexModel> {
    IndexBackend::from_manifest(id.to_string(), m, version, tag, assets, manifest)
}
...
/// instead.
fn ensure_unique_sources<M>(backends: &[IndexBackend<M>]) -> anyhow::Result<()> {
...
fn ensure_unique_backend_ids<M>(backends: &[IndexBackend<M>]) -> anyhow::Result<()> {
    let mut seen: std::collections::HashMap<&str, &str> = std::collections::HashMap::new();
    for b in backends {
...
mod tests {
    ...
    fn backend(id: &str, source: &str) -> IndexBackend<()> {
        IndexBackend {
            id: id.into(),
            backend_id: None,
            source: source.into(),
    ...
    fn duplicate_backend_ids_are_rejected() {
        ...
        a.backend_id = Some("com.example.big".into());
        let mut b = backend("openai", "github.com/x/openai");
        b.backend_id = Some("com.example.big".into());

        let err = ensure_unique_backend_ids(&[a, b]).unwrap_err();
        let msg = err.to_string();
        assert!(msg.contains("duplicate backend id"), "{msg}");
        assert!(msg.contains("com.example.big"), "{msg}");
        assert!(msg.contains("mistral") && msg.contains("openai"), "{msg}");
    }

    #[test]
    fn distinct_backend_ids_pass() {
        let mut a = backend("mistral", "github.com/x/mistral");
        a.backend_id = Some("com.example.small".into());
        let mut b = backend("openai", "github.com/x/openai");
        b.backend_id = Some("com.example.big".into());
    ...
    fn entries_without_a_backend_id_never_collide() {
        let backends = vec![
            ...
            backend("openai", "github.com/x/openai"),
        ];
