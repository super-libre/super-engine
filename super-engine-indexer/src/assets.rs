...
fn validate_tarball_read<R: Read>(
    ...
    for entry in archive.entries()? {
        ...
        // Accept the entrypoint at its declared path (e.g. `bin/backend`),
...
mod tests {
    ...
    fn accepts_tarball_with_bin_entrypoint() {
        let bytes = make_tarball(|tb| {
            ...
            h.set_mode(0o755);
            h.set_cksum();
            tb.append_data(&mut h, "bin/backend", &b"abc"[..]).unwrap();
        });
        validate_tarball("v.tar.gz", "backend", &bytes).unwrap();
    }
    ...
    fn accepts_tarball_with_path_entrypoint() {
        ...
        validate_tarball("q.tar.gz", "bin/backend", &bytes).unwrap();
    }
    ...
    fn rejects_output_over_the_unpack_budget_at_publish() {
        ...
        let bytes = make_tarball(|tb| {
            ...
            h.set_mode(0o755);
            h.set_cksum();
            tb.append_data(&mut h, "bin/backend", &body[..]).unwrap();
        });
        // total_cap = 100 < the 200-byte entry.
        let err = validate_tarball_read("v.tar.gz", "backend", &bytes[..], 100).unwrap_err();
        assert!(
            matches!(err, AssetError::TarBudget { .. }),
            "expected TarBudget, got {err:?}"
        );
        // The same archive validates when the cap is generous.
        validate_tarball_read("v.tar.gz", "backend", &bytes[..], 10_000).unwrap();
    }
    ...
    fn rejects_tarball_without_entrypoint() {
        ...
        let err = validate_tarball("v.tar.gz", "backend", &bytes).unwrap_err();
    ...
    fn validates_tarball_reassembled_from_parts() {
        ...
        validate_subprocess_parts(&[p0, p1], "q.tar.gz", "bin/backend").unwrap();
    }
    ...
    fn rejects_parts_in_wrong_order() {
        ...
        assert!(validate_subprocess_parts(&[p1, p0], "q.tar.gz", "bin/backend").is_err());
    }
}
