...
//!
//! `<indexer> local --out <dir> [--base-url <url>] [--allow-missing-assets] <backend.toml>...`
...
use super_engine_spec::index::{
    Index, IndexAsset, IndexAssets, IndexBackend, MIN_CLIENT, SCHEMA_VERSION,
};
...
pub fn run<P: Product>(args: &LocalArgs) -> anyhow::Result<()> {
    ...
    for path in &args.manifests {
        let b = build_entry::<P>(path, &args.out, base, args.allow_missing_assets)
    ...
    super_engine_spec::fs::write_atomic(&out_path, text.as_bytes())
...
fn build_entry<P: Product>(
    path: &Path,
    out_dir: &Path,
    base: &str,
    allow_missing: bool,
) -> anyhow::Result<IndexBackend<P::IndexModel>> {
    let text =
        std::fs::read_to_string(path).with_context(|| format!("reading {}", path.display()))?;
    let m = Manifest::<P>::parse(&text)?;
...
mod tests {
    ...
    fn builds_a_wasm_index_with_placeholder_when_asset_missing() {
        ...
        run::<TestProduct>(&args).unwrap();

        let json = std::fs::read_to_string(dir.path().join("index.json")).unwrap();
        let index: Index<<TestProduct as Product>::IndexModel> =
