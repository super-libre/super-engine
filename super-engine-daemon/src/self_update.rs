...
//! Self-update checking: whether the product has a newer release than the
//! daemon running, and which installer to offer for it. Contract: each
...
use super_engine_protocol::models::self_update::{InstallerAsset, SelfUpdateStatus};
...
///
/// Public for the daemons' own tests of what they do with a failed check.
pub const UNREACHABLE_ENDPOINT: &str = "http://127.0.0.1:1";
...
fn effective_beta_optin_for(current: &str, optin: UpdateBetaOptIn) -> bool {
    match optin {
        ...
        UpdateBetaOptIn::Auto => {
            super_engine_spec::version::parse_version(current).is_some_and(|v| !v.pre.is_empty())
        }
    }
}

fn select_candidate(releases: &[Release], include_prereleases: bool) -> Option<&Release> {
    releases
        .iter()
        ...
        .filter_map(|r| super_engine_spec::version::parse_version(&r.tag).map(|v| (v, r)))
...
fn find_installer_asset<'a>(
    ...
    let want = installer_name(product, triple);
...
async fn resolve_installer_asset(
    ...
    let asset = find_installer_asset(product, release, triple)?;
    ...
    let sums = super_engine_spec::verify::parse_sha256sums(&text);
...
pub struct SelfUpdateChecker {
    /// Whose releases to check.
    product: &'static ProductSpec,
    /// The version of the daemon running, e.g. `0.3.1`.
...
impl SelfUpdateChecker {
    /// A checker for `product`'s releases, for a daemon at `current_version`
    /// (its `CARGO_PKG_VERSION`), with no check run yet.
    #[must_use]
    pub fn new(product: &'static ProductSpec, current_version: &'static str) -> Self {
        Self {
            product,
            current_version,
    ...
    /// Whether `optin` puts this daemon on the prerelease channel: `auto`
    /// does when the running version is itself a prerelease.
    #[must_use]
    pub fn effective_beta_optin(&self, optin: UpdateBetaOptIn) -> bool {
        effective_beta_optin_for(self.current_version, optin)
    }
    ...
    pub async fn status_for(&self, optin: UpdateBetaOptIn) -> SelfUpdateStatus {
        ...
        if state.resolved_include_pre.is_none() {
            status.beta_optin_effective = self.effective_beta_optin(optin);
        }
        status
    }
    ...
    ///
    /// # Panics
    /// Never in practice: the product's `repo` is a hardcoded, valid
    /// `<host>/<owner>/<repo>` reference, which the protocol's tests check.
    pub async fn run_check(
        ...
        let include_pre = self.effective_beta_optin(optin);
        let repo = RepoRef::parse(self.product.repo).expect("static repo ref");
        ...
        let new_checker_state = match result {
            Ok(releases) => {
                ...
                let update_available = candidate.is_some_and(|c| {
                    super_engine_spec::version::update_available(self.current_version, &c.tag)
                });
                let installer = match (update_available, candidate, target_triple()) {
                    (true, Some(c), Some(t)) => {
                        resolve_installer_asset(self.product, client, c, t).await
                    }
                    _ => None,
                };
                CheckerState {
...
mod tests {
    ...
    async fn run_check_success_and_failure_paths() {
        ...
        let gh = github(UNREACHABLE_ENDPOINT.to_string());
    ...
    async fn pre_check_status_reports_the_configured_channel() {
        ...
        assert_eq!(
            checker
                ...
                .await
                .beta_optin_effective,
            checker.effective_beta_optin(UpdateBetaOptIn::Auto)
        );
    }
