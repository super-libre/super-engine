...
//! The session tokens a daemon has issued, and their copy in the keyring.
...
///
/// `3` moved the grantee from a flat `exe_path` + `flatpak_app_id` pair to a
/// tagged [`PeerIdentity`], so that a token minted for a web origin cannot be
/// stored in fields that mean "binary". Version 2 blobs are migrated on load by
/// [`V2TokenMeta`] rather than discarded — a version mismatch otherwise empties
/// the store, and every third-party client would face a fresh consent popup
/// after what is, to the user, an ordinary update.
...
/// Without it, an unavailable keyring is fatal: the daemon refuses to start
/// rather than silently run without session persistence.
pub const ALLOW_NO_KEYRING: &str = "ALLOW_NO_KEYRING";

fn allow_no_keyring(product: &ProductSpec) -> bool {
    std::env::var(product.env(ALLOW_NO_KEYRING)).is_ok_and(|v| v == "1")
}
...
pub async fn flush_persisted_sessions(timeout: Duration) {
...
async fn persist_loop(
    ...
    while let Some(first) = rx.recv().await {
        ...
        if let Some(snapshot) = snapshot {
            persist_snapshot(snapshot, keyring).await;
        }
...
#[cfg(not(test))]
async fn persist_snapshot(snapshot: HashMap<String, TokenMeta>, keyring: Keyring) {
    ...
    match tokio::task::spawn_blocking(move || keyring.set_sessions_blob(&json)).await {
...
pub struct TokenMeta {
    /// The name the caller declared when it asked. Persisted for diagnostics.
    pub app_name: String,
    /// The scopes the user granted, sorted and deduplicated.
    ...
    pub grantee: PeerIdentity,
    /// When the user granted it. Persisted for diagnostics.
...
impl TokenMeta {
    ...
    /// into each other. Within a native grantee both halves still have to
    /// match: every flatpak resolves to a path inside its own sandbox, so two
    /// different sandboxed apps can present the identical `/app/bin/<name>` and
    /// would otherwise share each other's sessions.
    #[must_use]
    ...
    #[must_use]
    pub fn describe_grantee(&self) -> String {
    ...
    fn from(old: V2TokenMeta) -> Self {
        Self {
            ...
            grantee: PeerIdentity::Native {
                ...
                flatpak_app_id: old.flatpak_app_id,
            },
...
impl TokenStore {
    ...
    /// (test builds).
    fn empty(keyring: Keyring) -> Self {
        Self {
            inner: Arc::new(Mutex::new(HashMap::new())),
            persist: SessionPersister::spawn(keyring),
        }
    }

    /// Load `product`'s persisted sessions from `keyring`, prune anything
    ...
    /// validate sessions. Set the product's [`ALLOW_NO_KEYRING`] variable to
    /// downgrade that to an ephemeral in-memory store for headless / CI hosts.
    ///
    /// # Errors
    /// Returns an error when the system keyring is unavailable and the
    /// product's [`ALLOW_NO_KEYRING`] variable is not set, so the daemon
    /// aborts startup.
    ///
    /// # Panics
    /// If a thread panicked while holding the store's lock.
    pub fn load_persisted(product: &ProductSpec, keyring: Keyring) -> anyhow::Result<Self> {
        let store = Self::empty(keyring);

        let blob = match keyring.get_sessions_blob() {
            Ok(Some(b)) => b,
            Ok(None) => {
                info!("No persisted sessions found; starting with empty store");
                return Ok(store);
            }
            Err(e) if allow_no_keyring(product) => {
                warn!(
                    "System keyring unavailable ({e}); {}=1 set — \
                     starting with an ephemeral in-memory session store (sessions will \
                     not survive a daemon restart)",
                    product.env(ALLOW_NO_KEYRING)
                );
                return Ok(store);
            }
            Err(e) => {
                error!(
                    "System keyring is unavailable or missing ({e}). {} requires \
                     ...
                     {}=1 to start without persistence (headless/CI).",
        ...
        if pruned > 0 || loaded.migrated {
            ...
            if let Ok(json) = serde_json::to_string(&cleaned)
                && let Err(e) = keyring.set_sessions_blob(&json)
            {
    ...
    /// smoke tests in each daemon's `tests/http_smoke_full.rs` (which exercise
    /// this crate built without the `test` cfg flag).
    ///
    /// # Panics
    /// If a thread panicked while holding the store's lock.
    #[must_use]
    ...
    /// The session `token` names, or why there is none: `unknown` or
    /// `expired` (an expired token is evicted on the way).
    ///
    /// # Errors
    /// The `invalid_session` reason the caller answers with.
    ///
    /// # Panics
    /// If a thread panicked while holding the store's lock.
    pub fn validate(&self, token: &str) -> Result<TokenMeta, &'static str> {
...
mod schema_tests {
    ...
    fn a_v2_blob_migrates_to_a_native_grantee() {
        let loaded = parse_sessions_blob(&v2_blob("/usr/bin/old-client", None))
    ...
    /// The sandbox id has to survive the move too: without it, two flatpaks
    /// sharing an `/app/bin/<name>` path would collapse into one grantee and
...
/// The keyring-write cooldown and the persisted wrapper's own round trip.
#[cfg(test)]
mod persistence_tests {
    use super::{
        KEYRING_FAILURE_COOLDOWN, KEYRING_LAST_FAILURE, PeerIdentity, SESSIONS_SCHEMA_VERSION,
        SessionsFile, TokenMeta, clear_keyring_failure_flag, keyring_writes_in_cooldown,
        mark_keyring_failure,
    };
    use chrono::{DateTime, Duration as ChronoDuration, Utc};
    use std::collections::HashMap;
    ...
    /// Drives the `persist_snapshot` short-circuit that prevents
    ...
    fn sessions_file_round_trips_through_json() {
        ...
        assert_eq!(back.sessions["tok-b"].scopes, vec!["status".to_string()]);
    }
}
