...
//! One streaming-download loop shared by a daemon's registry install pipeline
//! and its model-file downloader.
//!
...
#[cfg(test)]
mod tests {
    use super::{StreamError, stream_body_to_writer};
    use std::cell::Cell;

    /// `GET /body` from a mock server answering with `body`.
    async fn fetch(server: &mut mockito::Server, body: &[u8]) -> reqwest::Response {
        server
            .mock("GET", "/body")
            .with_body(body)
            .create_async()
    ...
    /// The bytes land in the writer, the count and the hash describe them,
    /// and progress adds up to the whole body.
    #[tokio::test]
    async fn the_body_is_written_hashed_and_counted() {
        let mut server = mockito::Server::new_async().await;
        let resp = fetch(&mut server, b"hello").await;
        let mut out = Vec::new();
        let reported = Cell::new(0);
        let (written, sha) = stream_body_to_writer(
            resp,
            &mut out,
            None,
            || false,
            |n| {
                reported.set(reported.get() + n);
            },
        )
        .await
        .expect("streams");
    ...
    /// Past the cap the download stops, before the chunk that crossed it
    /// reaches the writer.
    #[tokio::test]
    async fn a_body_over_the_cap_is_refused() {
        let mut server = mockito::Server::new_async().await;
        let resp = fetch(&mut server, &[0u8; 64]).await;
        let mut out = Vec::new();
        let err = stream_body_to_writer(resp, &mut out, Some(10), || false, |_| {})
            .await
            .expect_err("over the cap");
        assert!(matches!(err, StreamError::TooLarge { limit: 10 }), "{err}");
        assert!(
            out.len() <= 10,
            "wrote {} bytes past a 10-byte cap",
            out.len()
        );
    }
    ...
    async fn a_cancelled_download_stops() {
        ...
        let resp = fetch(&mut server, b"hello").await;
        let mut out = Vec::new();
        let err = stream_body_to_writer(resp, &mut out, None, || true, |_| {})
            .await
