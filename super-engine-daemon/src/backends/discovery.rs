...
//! manifest plus its entrypoint (a `.wasm` component or a native binary).
//! This module scans that directory and turns each manifest into a
//! [`DiscoveredBackend`] carrying fully-resolved [`ModelDefinition`]s.
...
use super_engine_spec::manifest::{
    BASE_URL_OPTION, Capabilities, Device, Manifest, ModelEntry, Opt, Secret,
};
...
#[derive(Clone, Debug)]
pub struct DiscoveredBackend<P: Product> {
    ...
    /// so the daemon's backend listing can report it: the registry index has
    /// one only for backends it lists, leaving a sideloaded backend with none
    /// anywhere.
    ...
    /// Declared secrets the backend expects as `x-{product}-secret-*` headers.
    pub secrets: Vec<Secret>,
    /// Declared options the backend accepts as `x-{product}-option-*` headers.
    pub options: Vec<Opt>,
    ...
    /// it is needed: header injection asks the same question on every settings
    /// write, and a disk read per write is not affordable.
    pub capabilities: Capabilities<P::Capabilities>,
    /// Models this backend serves.
    pub models: Vec<ModelDefinition<P::Model>>,
}
...
///
/// The same read backs the daemon's backend listing and the
/// `installed_version` in its registry listing, so the version a client sees
/// and the one an update is judged against cannot disagree.
#[must_use]
pub fn installed_version<P: Product>(dir: &Path) -> Option<String> {
    Manifest::<P>::load_installed(dir)
        .ok()
...
#[must_use]
pub fn discover<P: Product>(
    backends_dir: &Path,
) -> (Vec<DiscoveredBackend<P>>, Vec<DiscoveredBackend<P>>) {
...
/// directory and deleting one are different responsibilities.
fn dedup_sources<P: Product>(
    backends: Vec<DiscoveredBackend<P>>,
) -> (Vec<DiscoveredBackend<P>>, Vec<DiscoveredBackend<P>>) {
    use std::collections::HashMap;

    let mut groups: HashMap<String, Vec<DiscoveredBackend<P>>> = HashMap::new();
    for b in backends {
    ...
    for (source, mut group) in groups {
        group.sort_by(|a, b| {
            let av = super_engine_spec::version::parse_version(&a.version);
            let bv = super_engine_spec::version::parse_version(&b.version);
            bv.cmp(&av)
...
fn is_id_named<P: Product>(b: &DiscoveredBackend<P>) -> bool {
...
fn load_backend<P: Product>(dir: &Path) -> anyhow::Result<DiscoveredBackend<P>> {
    ...
    // user never wrote.
    let mut m = Manifest::<P>::load_installed(dir)?;
    super::validate_runtime(&m)?;
    ...
    for opt in &mut m.options {
        if opt.name == BASE_URL_OPTION && opt.default.take().is_some() {
            warn!(
...
///   contradiction.
fn validate_supported_devices<M>(entry: &ModelEntry<M>) -> anyhow::Result<Vec<Device>> {
...
/// backends may serve the same model `name`, so a name-only lookup would
/// resolve by scan order — which comes from `read_dir` and can differ between
/// runs — and a daemon that persists the backend it picked would then bind a
/// model to a different engine between runs. Callers that accept an omitted
/// `source` on the wire resolve it against the active backend first; an empty
/// `source` here matches nothing.
#[must_use]
pub fn find_model<'a, P: Product>(
    backends: &'a [DiscoveredBackend<P>],
    name: &str,
    source: &str,
) -> Option<(&'a DiscoveredBackend<P>, &'a ModelDefinition<P::Model>)> {
    backends
        .iter()
...
#[must_use]
pub fn dir_name<P: Product>(backend: &DiscoveredBackend<P>) -> Option<String> {
    backend
        .dir
        .file_name()
...
#[must_use]
pub fn list_models<P: Product>(backends: &[DiscoveredBackend<P>]) -> Vec<(String, String)> {
    backends
        .iter()
...
/// The default backends search directory: `<data_dir>/backends`.
#[must_use]
pub fn default_backends_dir(product: &ProductSpec) -> PathBuf {
