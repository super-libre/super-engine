...
//! The short tones a daemon plays as audio cues: a recording starting and
//! stopping, a theme preview. Each sequence opens the output device afresh,
//! so a device switched since the last cue is picked up.
...
/// The variable that, set to `1`, makes every cue a no-op:
/// `<PREFIX>_MUTE_CUES`.
///
/// An integration suite spawns a real daemon and exercises
/// `POST /audio_theme/test`, whose entire job is to make noise — so without
/// this, running the tests plays beeps out of the speakers of whoever ran
/// them. The endpoint's contract is that it accepts the request and reports
/// success, which is what a test asserts; the audio is not observable from a
/// test either way. Like `<PREFIX>_AUTO_APPROVE`, it is a "tests must not
/// reach into the user's session" switch.
#[must_use]
pub fn mute_cues_env(product: &ProductSpec) -> String {
    product.env("MUTE_CUES")
}

/// Whether `product`'s cues are muted for this process.
fn cues_muted(product: &ProductSpec) -> bool {
    std::env::var(mute_cues_env(product)).is_ok_and(|v| v == "1")
}

/// Play a sequence of beeps on a freshly initialized output device, unless
/// `product`'s cues are muted (see [`mute_cues_env`]).
///
/// # Errors
///
...
pub fn play_beep_sequence(
    ...
    if cues_muted(product) {
        log::debug!(
            "{}=1 — skipping cue {frequencies:?}",
            mute_cues_env(product)
        );
        return Ok(());
    }
...
mod tests {
    ...
    fn test_empty_frequencies() {
        ...
        let result = play_beep_sequence(&TEST, &[], 100, 10, 10, 1.0);
        assert!(
            result.is_ok(),
    ...
    /// The mute switch is named after the product, beside its other test
    /// switches.
    #[test]
