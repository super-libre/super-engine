...
//! install task then copies the directory into place instead of
//! downloading.
...
use super_engine_spec::index::{IndexAssets, IndexBackend, id_from_source};
...
pub fn resolve<P: Product>(local_path: &Path) -> Result<IndexBackend<P::IndexModel>, ResolveError> {
    ...
    let m = Manifest::<P>::load(local_path).map_err(anyhow::Error::from)?;
    crate::backends::validate_runtime(&m)?;
...
mod tests {
    ...
    /// [`super::resolve`] for the test product.
    fn resolve(
        local_path: &Path,
    ) -> Result<IndexBackend<super_engine_spec::test_product::IndexModel>, ResolveError> {
        super::resolve::<TestProduct>(local_path)
    }
    use std::fs;
...
"#;
    ...
    fn rejects_missing_dir() {
        let err = resolve(Path::new("/this/should/not/exist/super_engine_test")).unwrap_err();
