...
use super_engine_spec::manifest::{Kind, Manifest, ManifestError};
use super_engine_spec::product::Product;
use super_engine_spec::verify::MAX_MANIFEST_BYTES;
use thiserror::Error;
...
pub async fn resolve<P: Product>(
    client: &dyn ForgeClient,
    repo_url: &str,
) -> Result<IndexBackend<P::IndexModel>, ResolveError> {
    ...
    let manifest = Manifest::<P>::parse(&manifest_text)?;
    ...
    if !super_engine_spec::is_safe_component(&id) {
...
fn synthesize_assets<P: Product>(
    manifest: &Manifest<P>,
...
// The manifest types are the canonical `super_engine_spec::manifest` set (see
// the imports): custom_repo parses a remote repo's `backend.toml`
...
mod tests {
    ...
    async fn a_repo_with_no_published_release_names_the_repo_not_the_api_url() {
        ...
        server
            ...
            .create_async()
            .await;
        let gh = super_engine_forge::Github::new(server.url(), None, "super-engine-daemon-tests");

        let err = resolve::<TestProduct>(&gh, "https://github.com/o/b")
            .await
