...
pub async fn run<Prod: Product, F>(
    p: &Pipeline<F>,
    entry: &IndexBackend<Prod::IndexModel>,
    selection: &Selection,
    ...
    F: Fn(InstallPhase, Option<(u64, Option<u64>)>) + Send + Sync,
{
    ...
    let toml_text = fetch_verified_manifest::<Prod>(&p.http, entry, pin).await?;
    ...
    ensure_dir_free_for::<Prod>(&final_path, &entry.source)?;
    let inherit_from = previous_dir_for::<Prod>(&p.backends_dir, &entry.source)
        .await
        .unwrap_or_else(|| final_path.clone());
    preserve_models::<Prod>(&staging, &inherit_from)
        .await
        .map_err(|e| PipelineError::Io(e).as_typed(P::Installing))?;
...
async fn download_and_verify<M, F>(
    p: &Pipeline<F>,
...
/// # Panics
/// None.
pub async fn run_local<Prod: Product, F>(
    p: &Pipeline<F>,
    entry: &IndexBackend<Prod::IndexModel>,
    src_dir: &Path,
) -> Result<String, (InstallPhase, InstallError)>
...
fn copy_staged_backend(
    ...
    if !super_engine_spec::is_safe_relative_path(entrypoint) {
...
async fn find_serving<Prod: Product>(
    backends_dir: &Path,
    source: &str,
    exclude: Option<&Path>,
) -> Option<PathBuf> {
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        if Manifest::<Prod>::load(&dir).is_ok_and(|m| m.backend.source == source) {
            return Some(dir);
        }
    }
    None
...
async fn previous_dir_for<Prod: Product>(backends_dir: &Path, source: &str) -> Option<PathBuf> {
    find_serving::<Prod>(backends_dir, source, None).await
}
...
pub async fn retire_previous_dir<Prod: Product>(
    backends_dir: &Path,
    source: &str,
    keep: &Path,
) -> Option<PathBuf> {
    let dir = find_serving::<Prod>(backends_dir, source, Some(keep)).await?;
...
async fn preserve_models<Prod: Product>(staging: &Path, final_path: &Path) -> std::io::Result<u64> {
    let (Ok(old), Ok(new)) = (
        Manifest::<Prod>::load(final_path),
        Manifest::<Prod>::load(staging),
    ) else {
        return Ok(0);
    };
...
fn ensure_dir_free_for<Prod: Product>(
    final_path: &Path,
    source: &str,
) -> Result<(), (InstallPhase, InstallError)> {
    let Ok(occupant) = Manifest::<Prod>::load(final_path) else {
        return Ok(());
    };
...
async fn fetch_verified_manifest<Prod: Product>(
    http: &reqwest::Client,
    entry: &IndexBackend<Prod::IndexModel>,
    pin: &IndexAsset,
) -> Result<String, (InstallPhase, InstallError)> {
    ...
    verify_manifest_bytes::<Prod>(&bytes, &pin.sha256, entry)
}
...
fn verify_manifest_bytes<Prod: Product>(
    bytes: &[u8],
    pin_sha256: &str,
    entry: &IndexBackend<Prod::IndexModel>,
) -> Result<String, (InstallPhase, InstallError)> {
    use crate::backends::validate_runtime;
    use InstallPhase as P;
    ...
    let m = Manifest::<Prod>::parse(&text).map_err(|e| reject(&format!("parse: {e}")))?;
...
mod tests {
    ...
    use super_engine_spec::index::IndexAssets;
    use super_engine_spec::test_product::{self, TestProduct};

    type IndexBackend = super_engine_spec::index::IndexBackend<test_product::IndexModel>;
...
"#;
    ...
    fn pinned_manifest_verifies_and_installs_verbatim() {
        ...
        let out = verify_manifest_bytes::<TestProduct>(bytes, &sha_hex(bytes), &entry)
    ...
    fn pinned_manifest_hash_mismatch_is_rejected() {
        ...
        let err =
            verify_manifest_bytes::<TestProduct>(PINNED_MANIFEST.as_bytes(), "deadbeef", &entry)
                .unwrap_err();
    ...
    fn pinned_manifest_identity_spoof_is_rejected() {
        ...
        let err = verify_manifest_bytes::<TestProduct>(bytes, &sha_hex(bytes), &entry).unwrap_err();
    ...
    fn the_install_dir_is_the_backend_id_when_one_is_declared() {
        let e = index_entry("voxtral", Some("app.example.voxtral"));
        assert_eq!(install_dir_name(&e), "app.example.voxtral");
    }
    ...
    fn install_dir_name_never_escapes_the_backends_dir_when_joined() {
        let backends_dir = Path::new("/var/lib/example/backends");
    ...
    async fn run_rejects_unmatched_selection_without_panicking() {
        ...
        let err = run::<TestProduct, _>(&pipeline, &entry, &Selection::Subprocess { index: 0 })
            .await
            .unwrap_err();
        assert!(matches!(err.1, InstallError::Incompatible));
        // Wasm selection but the entry has no wasm asset.
        let err = run::<TestProduct, _>(&pipeline, &entry, &Selection::Wasm)
            .await
            .unwrap_err();
        assert!(matches!(err.1, InstallError::Incompatible));
    }
...
"#
        ...
        let moved = super::preserve_models::<TestProduct>(&staging, &final_path)
            .await
            .unwrap();
        assert_eq!(moved, 7);
    ...
    async fn preserve_models_is_a_no_op_without_an_installed_manifest() {
        ...
        std::fs::create_dir_all(&staging).unwrap();
        let moved = super::preserve_models::<TestProduct>(&staging, &root.path().join("absent"))
            .await
            .unwrap();
        assert_eq!(moved, 0);
    }
...
"#
    ...
    fn ensure_dir_free_for_allows_an_update_of_the_same_backend() {
        let root = tempfile::tempdir().unwrap();
        let dir = root.path().join("app.example.voxtral");
        occupied_backend_dir(&dir, "github.com/x/voxtral");
        ensure_dir_free_for::<TestProduct>(&dir, "github.com/x/voxtral")
            .expect("same source is an update");
    }
    ...
    fn ensure_dir_free_for_allows_an_unreadable_directory() {
        ...
        ensure_dir_free_for::<TestProduct>(&empty, "github.com/x/voxtral")
            .expect("a fresh install has no dir");
    }
    ...
    fn ensure_dir_free_for_refuses_a_directory_serving_another_backend() {
        ...
        let dir = root.path().join("app.example.voxtral");
        occupied_backend_dir(&dir, "github.com/x/voxtral");
        let err = ensure_dir_free_for::<TestProduct>(&dir, "github.com/someone/thing").unwrap_err();
    ...
    async fn run_local_refuses_to_install_over_a_different_backend() {
        ...
        let victim = backends_dir.join("app.example.voxtral");
...
"#,
        ...
        entry.id = "thing".into();
        entry.backend_id = Some("app.example.voxtral".into());
        ...
        let err = run_local::<TestProduct, _>(&pipeline, &entry, &staged)
            .await
...
"#;
    ...
    async fn retire_previous_dir_removes_the_superseded_directory() {
        let root = tempfile::tempdir().unwrap();
        let old = root.path().join("example-voxtral");
        let new = root.path().join("app.example.voxtral");
        ...
        std::fs::write(
            ...
            r#"
[backend]
source = "github.com/x/example-voxtral"
name = "Voxtral"
version = "1.0.0"
kind = "subprocess"
entrypoint = "voxtral"
...
"#,
        ...
        let removed = super::retire_previous_dir::<TestProduct>(
            root.path(),
            "github.com/x/example-voxtral",
            &new,
        )
        .await;
...
"#,
    ...
    async fn retire_previous_dir_skips_a_directory_whose_manifest_does_not_parse() {
        ...
        let keep = root.path().join("app.example.voxtral");
...
"#,
        ...
        let found =
            super::previous_dir_for::<TestProduct>(root.path(), "github.com/x/example-voxtral")
                .await;
...
"#,
    ...
    async fn a_migration_preserves_model_files_via_previous_dir_for() {
        let root = tempfile::tempdir().unwrap();
        let old_dir = root.path().join("example-voxtral");
        let staging = root.path().join(".staging/app.example.voxtral-1.0.1");
        let final_path = root.path().join("app.example.voxtral");
...
"#
        ...
        let inherit_from =
            super::previous_dir_for::<TestProduct>(root.path(), "github.com/x/voxtral")
                .await
                .unwrap_or_else(|| final_path.clone());
        assert_eq!(inherit_from, old_dir, "must inherit from the old directory");

        let moved = super::preserve_models::<TestProduct>(&staging, &inherit_from)
            .await
            .unwrap();
