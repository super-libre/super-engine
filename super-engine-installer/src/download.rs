...
/// lines on a real multi-hundred-MB tarball). Mirrors the
/// `PROGRESS_THROTTLE_BYTES` of each product app's updater — keep them in
/// step.
const PROGRESS_THROTTLE_BYTES: u64 = 256 * 1024;

/// Stream `url` to `dest` as `user_agent`, reporting throttled `(bytes_done, bytes_total)`
...
/// Fetch `url` as `user_agent`, as a UTF-8 string (used for the small `SHA256SUMS` listing),
...
mod tests {
    ...
    async fn download_string_returns_body_within_cap() {
        ...
        s.mock("GET", "/sums")
            ...
            .create_async()
            .await;
        let body = download_string(UA, &format!("{}/sums", s.url()), 1024)
            .await
            .unwrap();
    ...
    async fn download_string_enforces_its_cap() {
        ...
        s.mock("GET", "/big")
            ...
            .create_async()
            .await;
        let err = download_string(UA, &format!("{}/big", s.url()), 1024)
            .await
            .unwrap_err();
        assert!(matches!(err, InstallError::DownloadFailed(_)));
    }
