...
pub enum Reporter {
    Json,
    /// Text on a terminal, naming the product by its display name.
    Human(&'static str),
}
...
mod tests {
    ...
    // GOLDEN STRINGS — duplicated in each product app's parser tests.
    // Change one side and the other MUST change with it.
    #[test]
    fn json_events_serialize_to_documented_shape() {
        let ev = Event::Phase {
            phase: Phase::Download,
            message: "downloading super-test-x86_64-unknown-linux-gnu-beta.tar.gz",
        };
        assert_eq!(
            serde_json::to_string(&ev).unwrap(),
            r#"{"event":"phase","phase":"download","message":"downloading super-test-x86_64-unknown-linux-gnu-beta.tar.gz"}"#
        );
