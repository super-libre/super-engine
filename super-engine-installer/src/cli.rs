...
//! `<slug>-install`'s command line: the curl-bootstrap/interactive
...
#[must_use]
pub fn parse_cli(product: &ProductSpec) -> Cli {
    let m = Command::new(product.installer_binary())
        .about(format!(
            "Install or update {} (daemon, CLI, app, COSMIC applet)",
