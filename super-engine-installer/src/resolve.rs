...
use super_engine_spec::version::parse_version;

use crate::errors::InstallError;

/// Map a `std::env::consts::ARCH`-shaped string to a Rust target triple.
/// Only the architectures the products ship prebuilt Linux binaries for are
...
/// architectures the products ship prebuilt Linux binaries for are supported.
///
/// # Errors
/// [`InstallError::UnsupportedArch`] on anything else (e.g. `mips`).
...
/// The release tarball's asset name for product `slug` and `triple` at
/// `tag`, `<slug>-<triple>[-beta].tar.gz`: the `-beta` suffix is present
/// exactly when `tag` parses as semver with a non-empty prerelease component
/// (e.g. `v0.2.3-beta.1`).
#[must_use]
pub fn tarball_name(slug: &str, triple: &str, tag: &str) -> String {
    let is_beta = parse_version(tag).is_some_and(|v| !v.pre.is_empty());
    if is_beta {
        format!("{slug}-{triple}-beta.tar.gz")
    } else {
        format!("{slug}-{triple}.tar.gz")
    }
}
...
pub async fn resolve_target(
    ...
    let name = tarball_name(slug, triple, &release.tag);
...
mod tests {
    ...
    fn tarball_name_follows_channel_of_the_tag() {
        assert_eq!(
            tarball_name(TEST.slug, "x86_64-unknown-linux-gnu", "v0.2.3-beta.1"),
            "super-test-x86_64-unknown-linux-gnu-beta.tar.gz"
        );
        assert_eq!(
            tarball_name(TEST.slug, "x86_64-unknown-linux-gnu", "v0.2.2"),
            "super-test-x86_64-unknown-linux-gnu.tar.gz"
        );
    }
    ...
    async fn resolve_target_happy_path_returns_both_urls() {
        ...
        let releases_json = serde_json::json!([{
            ...
            "assets": [
                {
                    "name": "super-test-x86_64-unknown-linux-gnu-beta.tar.gz",
        ...
        let repo = super_engine_forge::RepoRef::parse(TEST.repo).unwrap();
        let client =
            super_engine_forge::Github::new(base.clone(), None, "super-engine-installer-tests");
    ...
    async fn resolve_target_errors_when_release_is_missing_sha256sums() {
        ...
        let releases_json = serde_json::json!([{
            ...
            "assets": [
                {
                    "name": "super-test-x86_64-unknown-linux-gnu.tar.gz",
                    "browser_download_url": format!("{base}/tarball"),
                    "size": 10,
                },
            ],
    ...
    fn required_asset_missing_names_the_asset() {
        ...
        let err = required_asset(&r, "super-test-x86_64-unknown-linux-gnu.tar.gz").unwrap_err();
        match err {
            crate::errors::InstallError::DownloadFailed(msg) => {
                assert!(
                    msg.contains("super-test-x86_64-unknown-linux-gnu.tar.gz"),
                    "{msg}"
                );
            }
