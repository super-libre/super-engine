...
//! Hermetic, offline end-to-end test: drives the real `super-test-install`
...
/// whatever machine runs this test (a dev box with the product already
...
/// `resolve::target_triple` does — duplicated here because that module is
/// private to the installer.
...
/// `super_engine_spec::verify::file_sha256_hex`).
...
/// requires (`super-test-install/src/stage.rs::build_manifest`): the three
/// daemon binaries, the systemd unit, the app binary + desktop + icon, and
/// the applet binary + desktop file + icon. At least one
/// `super-test-cosmic-applet-*.desktop` file is required whenever the applet
...
fn build_fixture_tarball(dir: &std::path::Path) -> Vec<u8> {
    ...
    std::fs::write(
        tree.join("systemd/super-test.service"),
        b"[Unit]\nDescription=fake\n",
    )
    .unwrap();
    std::fs::write(
        tree.join("resources/super-test-app.desktop"),
        b"[Desktop Entry]\nName=Super Test\n",
    )
    .unwrap();
    std::fs::write(
        tree.join("resources/icons/hicolor/scalable/apps/super-test-app.svg"),
        b"<svg/>",
    )
    .unwrap();
    std::fs::write(
        tree.join("resources/super-test-cosmic-applet-full.desktop"),
        b"[Desktop Entry]\nName=Super Test Applet\n",
    )
    .unwrap();
    std::fs::write(
        tree.join("resources/icons/hicolor/scalable/apps/super-test-cosmic-applet.svg"),
        b"<svg/>",
    )
    .unwrap();
...
async fn dry_run_resolves_downloads_verifies_and_stages_against_a_mocked_release() {
    ...
    let tarball_name = format!("super-test-{triple}-beta.tar.gz");
    ...
    let out = tokio::task::spawn_blocking({
        let base = base.clone();
        move || {
            std::process::Command::new(env!("CARGO_BIN_EXE_super-test-install"))
